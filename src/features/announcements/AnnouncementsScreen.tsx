...
import { CampusCard, StatusChip } from '../../components';
...
export const AnnouncementsScreen: React.FC = () => {
  ...
  const handleAttachmentDownload = (fileName: string) => {
    Alert.alert('Download Complete', `Saved attachment file successfully: ${fileName}`);
  };
  ...
  const filteredCirculars = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      if (sortBy === 'PRIORITY') {
        const priorityWeight = { 'Critical': 4, 'Urgent': 3, 'Important': 2, 'Normal': 1 };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
      ...
      {/* Tabs Selector at the very top, replacing the stats cards panel */}
...
const styles = StyleSheet.create({
  ...
  authorText: { fontSize: 12, color: Colors.AppOnSurfaceVariant, marginTop: 2 },
  dateText: { fontSize: 11, color: Colors.AppOnSurfaceVariant + '99' },
  annTitle: { fontSize: 15, fontWeight: '700', color: Colors.AppOnBackground, marginBottom: 6 },
