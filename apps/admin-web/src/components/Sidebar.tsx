    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-200 group relative ${
          isActive 
            ? 'bg-slate-100 text-slate-950 font-semibold border-l-4 border-blue-600' 
            : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
        }`}
      >
        <Icon className={`w-5 h-5 transition-transform duration-200 group-hover:scale-110 ${isActive ? 'text-blue-600' : 'text-slate-400'}`} />
    ...
    <aside className="w-72 fixed left-0 top-0 h-screen bg-white border-r border-slate-200 flex flex-col py-6 px-4 z-40 overflow-y-auto">
      ...
      <div className="px-2 mb-8 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Admin Hub</p>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="space-y-1">
          <div className="px-4 py-1 text-xs font-bold uppercase tracking-wider text-outline">
            Academic Control
          </div>
